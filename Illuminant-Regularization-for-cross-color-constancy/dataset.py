...
from utils import k_fold,AwbAug,AwbAug_mapping
...
class CcData(Dataset):
    def __init__(self, path, train=True, fold_num=0, test_sensor=''):
        ...
        self.data_aug = AwbAug_mapping(self.illu_full, sensor_name=test_sensor)
    ...
    def __getitem__(self, idx):
        ...
        return img_data.astype(np.float32), gd_data.astype(np.float32)
...
class CcDataEval(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img_data.ndim == 3 and img_data.shape[2] == 3:
            img_data = img_data.transpose(2, 0, 1)
        return img_data.astype(np.float32), gd_data.astype(np.float32),img_name
