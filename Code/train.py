...
from model import OneNet_PhysicsResidual
...
def train_net(net,
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            img_data, illu_gd = batch
            img_data = img_data.to(device)
            illu_gd = illu_gd.to(device)

            optimizer.zero_grad()
            illu_pred= net(img_data)
        ...
        if (epoch + 1) % validationFrequency == 0:
            ...
            plot_and_save_loss_curve(train_loss_list, val_loss_list, epoch_list,
                                     save_path=f'debug_models/loss_curve_{test_sensor}.png')
            ...
            if val_loss.avg < best_val_error:
                ...
                save_path = (f"debug_models/{dir_data.split('/')[-2]}"f"_best_epoch{epoch + 1}.pth")
...
def plot_and_save_loss_curve(train_losses, val_losses, epochs, save_path='loss_curve.png'):
...
def get_args():
    ...
    parser.add_argument('-trd', '--training_dir', dest='trdir', type=str, default='./TAU_S_IMX135/',
                        help='Training dataset directory')

    parser.add_argument('-ts', '--testing_sensor', dest='test_sensor', type=str, default='olympus',
...
if __name__ == '__main__':
    ...
    if not os.path.exists(log_dir):
        os.mkdir(log_dir)
    logging.info('Training of cross-sensor color constancy')
